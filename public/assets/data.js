(() => {
  ...
  function build(){
    ...
    return {
      ...
      categories,
      products:acc.map((p,i)=>({id:p.id,category:p.category,name:p.name,price:Number(p.price)||0,status:statusFor(p),image:imageFor(p,i),description:p.description||'',featured:p.featured===undefined?i<4:!!p.featured})),
